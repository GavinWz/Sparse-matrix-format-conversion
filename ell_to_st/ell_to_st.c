...
int ell_to_st(double **eData, int **offset, int n_row, int n_max, int** ist, int** jst, double** ast){
    ...
    for(int i = 0; i < n_row; i++){
        for(int j = 0; j < n_max; j++){
            if(*((int*)offset + i * (n_row - 1) + j) == -1)
                break;
            i_st[index] = i;
            j_st[index] = *((int*)offset + i * (n_row - 1) + j);
            a_st[index] = *((double*)eData + i * (n_row - 1) + j);
            index++;
        }
    }
    *ist = i_st;
...
//     int* ist,           //st格式行偏移数组
//     int* jst,           //st格式列偏移数组
//     double* ast,        //st格式非零元数组
//     int n_val,          //st格式非零元个数
//     int n_row,          //原二维矩阵行数
//     int n_col,          //原二维矩阵列数
//     int max,            //用户定义ELL格式行中非零元个数最大值
//     double*** eData,    //二维数组的指针，保存HYB中ELL数据矩阵
//     double*** offset,   //二维数组的指针，保存HYB中ELL列偏移矩阵
//     int** e_ist,        //一维数组的指针，保存HYB中st格式的行偏移数组
//     int** e_jst,        //一维数组的指针，保存HYB中st格式的列偏移数组
//     double**e_ast,      //一维数组的指针，保存HYB中st格式的非零元素数组
//     int* e_n_val        //整型变量指针，保存HYB中st格式的非零元素个数
// );
